...
export const resolvers = {
	...
	Mutation: {
		...
		createTask: async (_: any, _args: { input: any}, context: Context) => {
			...
			const createdActors = [];
		    const createdEscalators = [];

			const { name, email, phone} = assigned_to
			createdActors.push({
				name,
				email,
				phone,
			...
			// Populate createdEscalators array

			const { name: escalatorName, email: escalatorEmail, phone: escalatorPhone} = escalator

			createdEscalators.push({
				name:escalatorName,
				email: escalatorEmail,
				phone: escalatorPhone,
				
			})
			console.log(createdActors)
			console.log(createdEscalators)
			...
			const newTask = await context.prisma.task.create({
				data: {
					...taskData,
					assigned_to: {
						create: createdActors,
						// connect: { id: taskData.id}
					},
					escalator: {
						create: createdEscalators,
						// connect: { id: taskData.id}
					}
				},
				include: {
